CREATE TABLE IF NOT EXISTS weekly_leaderboard_records (
    ...
    CONSTRAINT pk_weekly_leaderboard_records PRIMARY KEY (user_id),
    CONSTRAINT fk_weekly_leaderboard_records_user
        FOREIGN KEY (user_id) REFERENCES users (id)
        ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    COMMENT = '유저별 주간 리더보드 등재 기록';

CREATE INDEX idx_weekly_leaderboard_records_best_first_reaction_time
    ON weekly_leaderboard_records (best_first_reaction_time);

CREATE INDEX idx_weekly_leaderboard_records_best_second_reaction_time
    ON weekly_leaderboard_records (best_second_reaction_time);

CREATE INDEX idx_weekly_leaderboard_records_best_competition_rate
    ON weekly_leaderboard_records (best_competition_rate);